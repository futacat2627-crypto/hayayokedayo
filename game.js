...
// はやと弾幕配列
let hayatoPurpleBullets  = []; // 紫：ランダム移動+拡散弾を自ら撃つ
...
function resetGame() {
  ...
  crimsonBullets = [];
  // はやとモード弾幕クリア
  hayatoPurpleBullets = []; hayatoNavyBullets = []; hayatoBlueBars = [];
...
function spawnHayatoPurple() {
  ...
  hayatoPurpleBullets.push({
    ...
    size: 10,
    lastSpread: Date.now(),
    spreadInterval: 800 + Math.random() * 400
  });
}
...
function useStop() {
  skillsUsed[0] = true;
  if (hayatoMode) skillReloadTime[0] = Date.now();
  stopActive = true;
...
function useTeleport() {
  skillsUsed[1] = true;
  if (hayatoMode) skillReloadTime[1] = Date.now();
...
function useRoar() {
  skillsUsed[3] = true;
  if (hayatoMode) skillReloadTime[3] = Date.now();
  bullets = [];
  blueBullets = [];
  yellowBullets = [];
  whiteBullets = [];
...
function update() {
  ...
  if (!hayatoMode) {
    ...
    if (boss.phase === 3 && phase3Time) {
      ...
      if (t >= 10) spreadCooldown = sakiMode ? 267 : 400;
    ...
    if (hayatoMode) {
      ...
      hayatoPurpleBullets.forEach(b => {
        ...
        if (Date.now() - b.lastSpread > b.spreadInterval) {
          ...
          const n = 24, base = Math.random() * Math.PI * 2;
          for (let i = 0; i < n; i++) {
            const a = base + Math.PI * 2 / n * i;
            bullets.push({ x: b.x, y: b.y, vx: Math.cos(a) * 3, vy: Math.sin(a) * 3, size: 5 });
          }
        }
      });
...
function draw() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (!skillsUsed[i]) {
      ...
      ctx.globalAlpha = 1;
    } else if (hayatoMode && skillReloadTime[i] > 0) {
      // リロード中：残り秒数を表示
...
function drawJoystick() {
  ...
  const kx = joystick.active ? joystick.touchX : bx;
  const ky = joystick.active ? joystick.touchY : by;
...
canvas.addEventListener("pointerdown", e => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (pos.x >= r.x - pad && pos.x <= r.x + r.w + pad &&
      ...
      if (!skillsUsed[i]) {
        [useStop, useTeleport, useReverse, useRoar][i]();
      }
      return;
    }
  }
  ...
  // ── ジョイスティック（左半分）──
  if (pos.x < canvas.width / 2 && joystickPointerId === null) {
